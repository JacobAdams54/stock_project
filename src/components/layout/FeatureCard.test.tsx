...
jest.mock("@mui/material", () => {
  return {
    Card: ({ children, ...rest }: any) => <div data-testid="mui-card" {...rest}>{children}</div>,
    CardContent: ({ children, ...rest }: any) => <div data-testid="mui-cardcontent" {...rest}>{children}</div>,
  };
});
...
const IconA = (props: any) => (
  <svg data-testid="icon-a" {...props} aria-hidden="false">
    <title>Icon A</title>
    <circle cx="8" cy="8" r="6" />
  </svg>
);
const IconB = (props: any) => (
  <svg data-testid="icon-b" {...props} aria-hidden="false">
    <title>Icon B</title>
    <rect width="12" height="12" />
  </svg>
);
const IconC = (props: any) => (
  <svg data-testid="icon-c" {...props} aria-hidden="false">
    <title>Icon C</title>
...
describe("FeatureCards (TDD tests)", () => {
  ...
  test("renders section landmark and custom heading + subheading", () => {
    ...
    render(<FeatureCards features={features} heading={heading} subheading={subheading} />);
  ...
  test("renders responsive grid and correct number of feature cards (uses MUI Card)", () => {
    ...
    const gridEl =
      container.querySelector(".grid") || Array.from(container.querySelectorAll("[class]")).find((el) => {
        const cls = el.getAttribute("class") || "";
        return cls.includes("grid-cols-1") || cls.includes("md:grid-cols-3") || cls.includes("grid-cols-");
      });
  ...
  test("each card shows icon, title, description, and styled icon badge", () => {
    ...
    features.forEach((feat) => {
      ...
      const iconTestId =
        feat.title === "Real-time Data" ? "icon-a" : feat.title === "AI Predictions" ? "icon-b" : "icon-c";
      ...
      const badge = Array.from(container.querySelectorAll("[class]")).find((el) => {
        const cls = el.getAttribute("class") || "";
        return cls.includes(feat.bgColor.split("-").slice(0, 2).join("-")) || cls.includes(feat.bgColor);
      });
      ...
      expect(icon.getAttribute("class") || "").toMatch(new RegExp(feat.color.split("-").slice(0, 2).join("-")));
    });
  });
